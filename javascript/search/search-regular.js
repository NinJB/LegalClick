const search = Vue.createApp({
  ...
  methods: {
    ...
    fetchLawyerServicesList() {
      fetch('http://localhost:5500/api/lawyer-services')  // expects array here
      .then(res => res.json())
      .then(data => {
    ...
    <div v-if="showMismatchPopup" class="popup-overlay" @click.self="closeMismatchPopup">
      <div class="complete-setup">
        ...
        <div class="buttons-setup">
          ...
          <a :href="'/html/lawyer/specialization.html?role_id=' + loggedInRoleId"><button class="proceed">Finish Set-Up</button></a>
        </div>
      </div>
    </div>
  `
